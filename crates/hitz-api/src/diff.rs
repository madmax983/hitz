...
impl CalculateDiff for MetricsSnapshot {
    ...
    fn diff(&self, previous: &Self) -> Option<Self::Diff> {
        ...
        // ⚡ Bolt Optimization: Use explicit `for` loop to avoid `extend` reallocation overhead
        // even with `with_capacity`.
        ...
        for current_net in &self.networks {
            if let Some(prev_net) = previous.networks.iter().find(|n| n.interface == current_net.interface) {
