...
impl CalculateDiff for MetricsSnapshot {
    ...
    fn diff(&self, previous: &Self) -> Option<Self::Diff> {
        ...
        // ⚡ Bolt Optimization: Removed `Vec::with_capacity().extend()` and replaced
        // with iterator `.collect()`. The previous code used `filter_map()`, which
        // obscures the lower bound size hint. Because the iterator size is unknown,
        // `.extend()` was silently falling back to a dynamic reallocation path, completely
        // defeating the `with_capacity` intent and replacing 1 allocation with multiple.
        let disks: Vec<DiskRate> = self
            .disks
            .iter()
            .filter_map(|current_disk| {
        ...
        // ⚡ Bolt Optimization: Replaced `with_capacity().extend()` with `.collect()`.
        let networks: Vec<NetRate> = self
            .networks
            .iter()
            .filter_map(|current_net| {
