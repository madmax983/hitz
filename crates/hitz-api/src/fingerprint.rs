...
///
/// # Abstract
/// Encapsulates the string ID that quantizes a VM's continuous telemetry
/// into specific buckets (CPU, RAM, Disk, Network) to enable pattern matching.
///
/// ## Examples
/// ```rust
...
///
/// # Abstract
/// Types implementing this trait can convert their complex internal metrics
/// state into a simple, discrete [`VmFingerprint`] string identifier.
pub trait FingerprintGenerator {
    ...
    /// let snap = MetricsSnapshot {
    ///     timestamp_ms: 1000,
    ///     cpu: CpuMetrics { total_pct: 50.0, per_core: vec![50.0], load_avg: [1.0, 0.5, 0.2] },
    ...
    /// assert_eq!(fp.id, "FP-C5-R5-D0-N0");
    /// ```
