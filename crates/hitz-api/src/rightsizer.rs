...
///
/// # Abstract
/// Exposes logic to identify over-provisioned or under-provisioned resources
/// based on recent usage telemetry.
pub trait RightSizer {
    /// Analyzes current metrics against the VM's configuration and returns recommendations.
    ///
    /// # Abstract
    /// Compares a telemetry snapshot to the baseline VM configuration to produce
    /// actionable scaling recommendations for cost-savings or performance.
    ///
    /// # Details
    /// Recommendations may include scaling up CPU/RAM if nearing exhaustion, or
    /// scaling them down if heavily underutilized.
    ///
    /// ## Examples
    ///
    /// ```rust
