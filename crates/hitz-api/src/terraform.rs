...
///
/// # Abstract
/// Provides a standardized way to convert internal configuration structs
/// into Infrastructure-as-Code definitions.
///
/// ## Examples
///
/// ```rust
...
pub trait ToTerraform {
    ...
    ///
    /// # Abstract
    /// Generates the raw HCL string for the implementing configuration,
    /// using the provided `resource_name` as the Terraform identifier.
    ///
    /// ## Examples
    ///
    /// ```rust
    ...
    /// };
    ///
    /// let hcl = config.to_terraform("worker_node");
    /// assert!(hcl.contains("worker_node"));
    /// ```
