...
impl ToTerraform for VmConfig {
    fn to_terraform(&self, resource_name: &str) -> String {
        // ⚡ Bolt: Pre-allocating `String::with_capacity(512)` prevents the underlying vector
        // from immediately reallocating on subsequent `writeln!` calls, eliminating a bottleneck.
