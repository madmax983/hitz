...
mod tests {
    ...
    #[test]
    fn test_to_terraform_complete() {
        let config = VmConfig {
            kernel_path: PathBuf::from("/boot/vmlinux"),
            initramfs_path: Some(PathBuf::from("/boot/initramfs.cpio")),
            disk_path: Some(PathBuf::from("/var/lib/hitz/disk.img")),
            ram_mib: 1024,
            cpus: 4,
        ...
        let hcl = config.to_terraform("prod_vm");
        assert!(hcl.contains("resource \"hitz_vm\" \"prod_vm\" {"));
        assert!(hcl.contains("kernel_path = \"/boot/vmlinux\""));
        assert!(hcl.contains("initramfs_path = \"/boot/initramfs.cpio\""));
        assert!(hcl.contains("disk_path = \"/var/lib/hitz/disk.img\""));
        assert!(hcl.contains("ram_mib = 1024"));
        assert!(hcl.contains("cpus = 4"));
        assert!(hcl.contains("cmdline = \"console=ttyS0 quiet\""));
