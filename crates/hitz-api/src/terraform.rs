...
mod tests {
    ...
    #[test]
    fn test_to_terraform_optional_fields() {
        let config = VmConfig {
            kernel_path: PathBuf::from("/boot/vmlinux"),
            initramfs_path: Some(PathBuf::from("/boot/init.cpio")),
            disk_path: Some(PathBuf::from("/dev/vda")),
            ram_mib: 512,
            cpus: 2,
        ...
        let hcl = config.to_terraform("test_vm_full");
        assert!(hcl.contains("resource \"hitz_vm\" \"test_vm_full\" {"));
        assert!(hcl.contains("initramfs_path = \"/boot/init.cpio\""));
        assert!(hcl.contains("disk_path = \"/dev/vda\""));
