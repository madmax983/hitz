...
impl ToTerraform for VmConfig {
    fn to_terraform(&self, resource_name: &str) -> String {
        // ⚡ Bolt Optimization:
        // Replaced initial `format!` with `String::with_capacity(256)`.
        // The `format!` macro allocates a buffer sized exactly for its content.
        // Subsequent `writeln!` calls would force immediate heap reallocations
        // as the string grew. Pre-allocating capacity avoids these reallocations.
