...
impl ToTerraform for VmConfig {
    fn to_terraform(&self, resource_name: &str) -> String {
        // ⚡ Bolt Optimization: Replacing `format!` with `String::with_capacity(256)` followed by `writeln!`
        // eliminates unnecessary heap reallocations as the string grows. `format!` allocates a buffer exactly
        // sized to the initial content, causing immediate reallocation on the very next append.
