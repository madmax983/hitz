...
pub fn analyze_vm(info: &VmInfo, metrics: &MetricsSnapshot) -> Vec<ResourceInsight> {
    ...
    // Process analysis
    if let Some(top_proc) = metrics
        .processes
        .first()
        .filter(|&top_proc| top_proc.cpu_pct > 80.0)
    {
        insights.push(ResourceInsight {
            level: WarningLevel::Warning,
            message: format!(
    ...
    let total_rx: u64 = metrics.networks.iter().map(|n| n.rx_errors).sum();
    let total_tx: u64 = metrics.networks.iter().map(|n| n.tx_errors).sum();

    if total_rx > 0 || total_tx > 0 {
        insights.push(ResourceInsight {
            level: WarningLevel::Warning,
            message: format!("Network errors detected (RX: {total_rx}, TX: {total_tx})"),
        });
    }
