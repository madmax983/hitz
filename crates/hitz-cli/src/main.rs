...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let msg = if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        ...
        if clean.is_empty() {
            ...
            let max_len = 200;
            // ⚡ Bolt Optimization: Use `char_indices().nth()` to find the truncation byte boundary
            // instead of `.chars().count()` and `.chars().take(n).collect::<String>()`.
            // This prevents iterating the string twice and eliminates an intermediate String allocation.
