...
fn format_metrics_snapshot(snap: &hitz_api::MetricsSnapshot) -> String {
    ...
    if !snap.disks.is_empty() {
        ...
        for disk in &snap.disks {
            ...
            let _ = disk_table.add_row([
                Cell::new(&disk.name),
                Cell::new(disk.reads_total),
                Cell::new(disk.writes_total),
                Cell::new(read_kb),
                Cell::new(write_kb),
            ]);
        }
    ...
    if !snap.networks.is_empty() {
        ...
        for net in &snap.networks {
            ...
            let _ = net_table.add_row([
                Cell::new(&net.interface),
                Cell::new(rx_kb),
                Cell::new(tx_kb),
                Cell::new(net.rx_packets),
                Cell::new(net.tx_packets),
            ]);
        }
    ...
    if !snap.processes.is_empty() {
        ...
        for proc in &snap.processes {
            ...
            let cpu_cell = Cell::new(format!("{:.1}%", proc.cpu_pct))
                .fg(color_for_pct(f64::from(proc.cpu_pct)));
