...
fn vm_state_cell(state: hitz_api::VmState) -> comfy_table::Cell {
    use comfy_table::{Cell, Color};
    match state {
        hitz_api::VmState::Running => Cell::new(state.as_str()).fg(Color::Green),
        hitz_api::VmState::Stopped => Cell::new(state.as_str()).fg(Color::Yellow),
        hitz_api::VmState::Failed => Cell::new(state.as_str()).fg(Color::Red),
        hitz_api::VmState::Created => Cell::new(state.as_str()).fg(Color::Cyan),
    }
}

fn health_status_cell(status: hitz_api::HealthStatus) -> comfy_table::Cell {
    use comfy_table::{Cell, Color};
    match status {
        hitz_api::HealthStatus::Healthy => Cell::new(" ✅ HEALTHY ")
            .fg(Color::White)
            .bg(Color::DarkGreen)
            .add_attribute(comfy_table::Attribute::Bold),
        hitz_api::HealthStatus::Warning => Cell::new(" ⚠️ WARN ")
            .fg(Color::Black)
            .bg(Color::Yellow)
            .add_attribute(comfy_table::Attribute::Bold),
        hitz_api::HealthStatus::Critical => Cell::new(" 🚨 CRIT ")
            .fg(Color::White)
            .bg(Color::DarkRed)
            .add_attribute(comfy_table::Attribute::Bold),
    }
...
async fn handle_vm_status(args: &VmIdArgs) -> Result<()> {
    ...
    if status.is_success() {
        ...
        if let Ok(info) = serde_json::from_str::<hitz_api::VmInfo>(&resp) {
            ...
            let state_cell = vm_state_cell(info.state);
...
fn handle_vm_timeline(args: &VmTimelineArgs) -> Result<()> {
    ...
    for (line_num, line_result) in reader.lines().enumerate() {
        ...
        if status_changed || reasons_changed {
            ...
            let status_cell = health_status_cell(health.status);
