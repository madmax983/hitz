    ...
    if status.is_success() {
        ...
        if let Ok(vms) = serde_json::from_str::<Vec<hitz_api::VmInfo>>(&resp) {
            ...
            for info in vms {
                ...
                let exit_reason_str = info.exit_reason.as_ref().map(|r| format!("{:?}", r)).unwrap_or("-".to_string());
                let exit_reason = &exit_reason_str;
                let _ = table.add_row([
                    Cell::new(&info.id),
                    state_cell,
        ...
        let _ = terminal.draw(|f| {
            ...
            if vms.is_empty() {
                ...
                let rows = vms.iter().map(|vm| {
                    ...
                    let exit_reason_str = vm.exit_reason.as_ref().map(|r| format!("{:?}", r)).unwrap_or("-".to_string());
                    let exit_reason = &exit_reason_str;
