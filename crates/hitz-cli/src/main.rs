...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let msg = if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        ...
        if let Some(msg) = v.get("message").and_then(|m| m.as_str()) {
            ...
            let parts_str = if let Some(obj) = v.as_object() {
                ...
                arr.iter().fold(String::new(), |mut acc, val| {
                    use std::fmt::Write;
                    let val_str = if let Some(s) = val.as_str() {
                        s.to_string()
                    } else {
            ...
            if parts_str.is_empty() {
                ...
                let max_len = 200;
                let display_text = if parts_str.chars().count() > max_len {
                    let truncated: String = parts_str.chars().take(max_len).collect();
                    format!("{}...", truncated)
                } else {
                    parts_str
                };
