...
fn format_error_response_inner(
    status: hyper::StatusCode,
    resp: &str,
    error_prefix: &str,
) -> String {
    if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        return format!("✗ {error_prefix}: {}", err.message);
    }
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(resp) {
        ...
        return format!("✗ {error_prefix} ({status}): {parts_str}");
    }
...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let msg = format_error_response_inner(status, resp, error_prefix);
    format!("\r\x1b[2K{}", msg)
}
