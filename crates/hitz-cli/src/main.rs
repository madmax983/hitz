...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let msg = if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        ...
        if clean.is_empty() {
            ...
            let max_len = 200;
            // ⚡ Bolt Optimization: Eliminated string double-iteration (`.chars().count()`)
            // and an intermediate heap allocation (`.chars().take(n).collect::<String>()`).
            // We now find the byte index in one pass and slice the original string.
