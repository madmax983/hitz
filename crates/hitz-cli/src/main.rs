...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let human_status = match status {
        hyper::StatusCode::BAD_REQUEST => "Invalid Request",
        hyper::StatusCode::NOT_FOUND => "Not Found",
        hyper::StatusCode::INTERNAL_SERVER_ERROR => "Internal Server Error",
        hyper::StatusCode::SERVICE_UNAVAILABLE => "Service Unavailable",
        hyper::StatusCode::CONFLICT => "Resource Conflict",
        _ => status.as_str(),
    };
    let msg = if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        ...
        if let Some(msg) = v.get("message").and_then(|m| m.as_str()) {
            ...
            if parts_str.is_empty() {
                format!("❌ {error_prefix} ({human_status})")
            } else {
                format!("❌ {error_prefix} ({human_status}): {parts_str}")
            }
        }
    } else {
        let clean = resp.trim();
        if clean.is_empty() {
            ...
            format!("❌ {error_prefix} ({human_status}): {display_text}")
        }
    };
    format!("\r\x1b[2K{}", msg)
}
