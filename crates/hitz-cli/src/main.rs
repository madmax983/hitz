...
fn format_metrics_snapshot(snap: &hitz_api::MetricsSnapshot) -> String {
    ...
    // ⚡ Bolt Optimization: Pre-allocate capacity for the UI output string to prevent
    // multiple dynamic heap reallocations during string formatting.
...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let msg = if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        ...
        if let Some(msg) = v.get("message").and_then(|m| m.as_str()) {
            ...
            // with an iterator chain using `.fold(String::with_capacity(128), ...)` to eliminate
            // intermediate heap allocations and multiple `format!` calls when formatting CLI error responses.
            let parts_str = v.as_object().map_or_else(
                || String::with_capacity(128),
                |obj| {
                    obj.iter()
                        .fold(String::with_capacity(128), |mut acc, (k, val)| {
                            use std::fmt::Write;
    ...
    for (line_num, line_result) in reader.lines().enumerate() {
        ...
        if status_changed || reasons_changed {
            ...
            let added_reasons = health
                ...
                .fold(String::with_capacity(128), |mut acc, reason| {
                    use std::fmt::Write;
                    if !acc.is_empty() {
                        let _ = write!(acc, "\n");
                    }
