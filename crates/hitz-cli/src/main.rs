...
fn run_vm(args: RunArgs) -> Result<ExitCode> {
    ...
    let boot_net = config.net.as_ref().map(|n| hitz_vmm::config::NetConfig {
        mac: n.mac.clone(),
    ...
    let boot_agent = match &config.guest_agent {
        hitz_api::GuestAgentMode::Auto => hitz_vmm::config::GuestAgentMode::Auto,
        hitz_api::GuestAgentMode::Custom(p) => hitz_vmm::config::GuestAgentMode::Custom(p.clone()),
        hitz_api::GuestAgentMode::Disabled => hitz_vmm::config::GuestAgentMode::Disabled,
    };

    let boot_config = hitz_vmm::config::VmConfig {
        ...
        cpus: config.cpus,
        cmdline: config.effective_cmdline().to_string(),
        net: boot_net,
