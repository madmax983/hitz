...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        return format!("\r\x1b[2K✗ {error_prefix}: {}", err.message);
    }

    if let Ok(v) = serde_json::from_str::<serde_json::Value>(resp) {
        if let Some(msg) = v.get("message").and_then(|m| m.as_str()) {
            return format!("\r\x1b[2K✗ {error_prefix}: {}", msg);
        }
        if let Some(err) = v.get("error").and_then(|e| e.as_str()) {
            return format!("\r\x1b[2K✗ {error_prefix}: {}", err);
        }
        ...
        if parts_str.is_empty() {
            return format!("\r\x1b[2K✗ {error_prefix} ({status})");
        }
        return format!("\r\x1b[2K✗ {error_prefix} ({status}): {parts_str}");
    }
    ...
    format!("\r\x1b[2K✗ {error_prefix} ({status}): {display_text}")
}
