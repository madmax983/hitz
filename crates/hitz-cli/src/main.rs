...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let msg = if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        ...
        if let Some(msg) = v.get("message").and_then(|m| m.as_str()) {
            ...
            // ⚡ Bolt Optimization: Replace `.fold(String::new(), ...)` chains
            // with `String::with_capacity(...)` to eliminate intermediate heap allocations
            // and avoid reallocations as the string grows.
            let parts_str = v.as_object().map_or_else(String::new, |obj| {
                let mut acc = String::with_capacity(obj.len() * 32);
                use std::fmt::Write;
...
fn handle_vm_timeline(args: &VmTimelineArgs) -> Result<()> {
    ...
    for (line_num, line_result) in reader.lines().enumerate() {
        ...
        if status_changed || reasons_changed {
            ...
            // with `String::with_capacity(...)` to eliminate intermediate heap allocations
            // and avoid reallocations as the string grows.
            let mut added_reasons = String::with_capacity(64);
            use std::fmt::Write;
            for reason in health
                .reasons
                .iter()
                .filter(|r| !current_reasons.contains(*r))
            {
                if !added_reasons.is_empty() {
                    let _ = write!(added_reasons, "\n");
                }
                let _ = write!(added_reasons, "+ {reason}");
            }

            let mut removed_reasons = String::with_capacity(64);
            for reason in current_reasons
                .iter()
                .filter(|r| !health.reasons.contains(*r))
            {
                if !removed_reasons.is_empty() {
                    let _ = write!(removed_reasons, "\n");
                }
                let _ = write!(removed_reasons, "- {reason}");
            }
