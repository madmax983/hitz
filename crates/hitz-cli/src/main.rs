    ...
    let msg = if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        ...
        if clean.is_empty() {
            ...
            let display_text = if clean.chars().count() > max_len {
                // ⚡ Bolt Optimization: Eliminated intermediate `String` (which allocated a `Vec` internally)
                // and instead formatted the truncated string directly from the chars iterator.
                let mut truncated = String::with_capacity(max_len + 3);
                for c in clean.chars().take(max_len) {
                    truncated.push(c);
                }
                truncated.push_str("...");
                truncated
