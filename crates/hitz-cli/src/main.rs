...
fn format_json_object(obj: &serde_json::Map<String, serde_json::Value>) -> String {
...
fn format_json_error(
    ...
    if let Some(msg) = v.get("message").and_then(|m| m.as_str()) {
        return format!("✗ {error_prefix}: {msg}");
    }
    if let Some(err) = v.get("error").and_then(|e| e.as_str()) {
        return format!("✗ {error_prefix}: {err}");
    }

    // ⚡ Bolt Optimization preserved inside extracted helper
    let parts_str = v.as_object().map_or_else(String::new, format_json_object);
...
fn format_plain_error(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let clean = resp.trim();
    if clean.is_empty() {
        return format!("✗ {error_prefix} ({status})");
    }
...
fn format_error_response(status: hyper::StatusCode, resp: &str, error_prefix: &str) -> String {
    let msg = if let Ok(err) = serde_json::from_str::<hitz_api::ApiError>(resp) {
        ...
        format_json_error(status, v, error_prefix)
    } else {
        format_plain_error(status, resp, error_prefix)
    };
    format!("\r\x1b[2K{}", msg)
}
