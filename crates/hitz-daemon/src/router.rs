    ...
    H: Hypervisor + Send + Sync + 'static,
{
    ...
    // ⚡ Bolt Optimization: Destructure `Request` into `parts` and `body` to avoid cloning `Method`
    // and heap allocating a `String` for the `path` string on the routing hot path.
    ...
    H: Hypervisor + Send + Sync + 'static,
{
    ...
    let create_req: CreateVmRequest = serde_json::from_slice(&body_bytes.to_bytes())
    ...
    H: Hypervisor + Send + Sync + 'static,
{
    let body_bytes = body
        ...
        .await
        .map_err(|e| DaemonError::Internal(format!("failed to read request body: {e}")))?;
    let action_req: ActionVmRequest = serde_json::from_slice(&body_bytes.to_bytes())
        .map_err(|e| DaemonError::Internal(format!("invalid JSON: {e}")))?;
    ...
    H: Hypervisor + Send + Sync + 'static,
{
    let body_bytes = body
        ...
        .await
        .map_err(|e| DaemonError::Internal(format!("failed to read request body: {e}")))?;
    let clone_req: CloneVmRequest = serde_json::from_slice(&body_bytes.to_bytes())
        .map_err(|e| DaemonError::Internal(format!("invalid JSON: {e}")))?;
