...
{
    // ⚡ Bolt Optimization: Avoid cloning `Method` or allocating `Uri` strings.
    // Use `req.into_parts()` to cheaply borrow `method` and `path`.
    ...
    H: Hypervisor + Send + Sync + 'static,
{
    ...
    let create_req: CreateVmRequest = serde_json::from_slice(&req_body.to_bytes())
    ...
    H: Hypervisor + Send + Sync + 'static,
{
    let req_body = body
        ...
        .await
        .map_err(|e| DaemonError::Internal(format!("failed to read request body: {e}")))?;
    let action_req: ActionVmRequest = serde_json::from_slice(&req_body.to_bytes())
        .map_err(|e| DaemonError::Internal(format!("invalid JSON: {e}")))?;
    ...
    H: Hypervisor + Send + Sync + 'static,
{
    let req_body = body
        ...
        .await
        .map_err(|e| DaemonError::Internal(format!("failed to read request body: {e}")))?;
    let clone_req: CloneVmRequest = serde_json::from_slice(&req_body.to_bytes())
        .map_err(|e| DaemonError::Internal(format!("invalid JSON: {e}")))?;
