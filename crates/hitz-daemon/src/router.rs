...
{
    // ⚡ Bolt Optimization: Avoid cloning the HTTP method or allocating the URI path string.
    // We use `req.into_parts()` to cheaply borrow `parts.method` and `parts.uri.path()`,
    // and pass the `body` ownership to sub-handlers.
    ...
    let result = async {
        match (&parts.method, parts.uri.path()) {
            ...
            (method, path) if path.starts_with("/vms/") => {
