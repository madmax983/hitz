    ...
    H: Hypervisor + Send + Sync + 'static,
{
    ...
    // It's cheaper to borrow it via `into_parts()` and pass the body to handlers.
...
{
    match (method, suffix) {
        (&Method::PUT, None) => handle_create(req_body, id, manager).await,
        ...
        (&Method::POST, Some("action")) => handle_action(req_body, id, manager).await,
        (&Method::POST, Some("clone")) => handle_clone(req_body, id, manager).await,
        _ => Ok(error_response(
