...
use hitz_api::{VmConfig, VmInfo, VmState, ExitReason};
use hitz_hal::Hypervisor;
use hitz_vmm::{SerialBuf, SerialReader};
                    ...
                    match result {
                        Ok(Ok(run_result)) => match run_result.exit_reason {
                            ExitReason::Halt | ExitReason::Shutdown | ExitReason::Canceled => {
                                ...
                                entry.exit_reason = Some(run_result.exit_reason.clone());
                            }
                        ...
                        Err(e) => {
                            entry.state = VmState::Failed;
                            entry.exit_reason = Some(ExitReason::Unexpected(format!("task panicked: {e}")));
                        }
                    }
