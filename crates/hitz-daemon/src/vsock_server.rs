...
struct GuestMetricsInstruments {
    cpu_usage: Gauge<f64>,
...
static CACHED_INSTRUMENTS: OnceLock<GuestMetricsInstruments> = OnceLock::new();

fn get_cached_instruments() -> &'static GuestMetricsInstruments {
    CACHED_INSTRUMENTS.get_or_init(|| {
        let meter = opentelemetry::global::meter("hitz");
        GuestMetricsInstruments {
            cpu_usage: meter
...
pub fn publish_to_otel(vm_id: &str, snap: &MetricsSnapshot) {
    let instruments = get_cached_instruments();
