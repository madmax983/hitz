...
impl MmioBus {
    ...
    pub fn register(&mut self, base_gpa: u64, size: u64, device: Box<dyn MmioDevice>) {
        let end_gpa = base_gpa.checked_add(size).expect("MMIO device range overflow");
        for slot in &self.slots {
            let slot_end = slot.base.checked_add(slot.size).expect("MMIO device range overflow");
            assert!(
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "MMIO device range overflow")]
