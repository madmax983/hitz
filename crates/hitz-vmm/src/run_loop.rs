...
/// advance_rip(&mut vcpu, 2).map_err(|_| hitz_hal::HalError::DeviceLockPoisoned)?;
/// assert_eq!(vcpu.get_regs().map_err(|_| hitz_hal::HalError::DeviceLockPoisoned)?.rip, 0x1002);
/// ```
...
/// advance_rip_with_rax(&mut vcpu, 1, 0x42).map_err(|_| hitz_hal::HalError::DeviceLockPoisoned)?;
/// let regs = vcpu.get_regs().map_err(|_| hitz_hal::HalError::DeviceLockPoisoned)?;
