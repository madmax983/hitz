# Phase 30: Guest Watchdog & Auto-Recovery — Spec

## 👤 User Story
As a Site Reliability Engineer (SRE), I want the system to automatically detect and restart unresponsive micro-VMs, so that service uptime is maintained without manual intervention and MTTR is minimized.

## 💼 The "So What?" (Business Problem)
Currently, our VMs emit telemetry, but if a guest kernel panics or the agent deadlocks, the VM remains in a "zombie" state indefinitely. Operators are forced to build external alerting and manual restart playbooks. This increases operational toil and severely impacts SLA compliance. By implementing an automated watchdog, we reduce operational cost and increase reliability, which is a key selling point for production workloads.

## 🎯 Metric Definition (Success)
- **Success:** Time to detect an unresponsive VM and initiate recovery is < 30 seconds for 99% of failures.
- **Success:** False positive rate for restarts is < 0.1% under heavy load.
