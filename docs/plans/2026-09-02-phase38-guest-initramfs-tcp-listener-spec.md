...
## 👤 User Story
As a Core Engineer, I want the test guest initramfs to run a TCP echo server on boot, so that I can write end-to-end integration tests that send packets from the host and verify the responses.
...
- **Functional Requirements:**
  ...
  - The initramfs must start a TCP service immediately after network interface initialization.
...
- **UDP Testing:** The focus is strictly on TCP. UDP will be handled in a separate phase if necessary.
